...
import { Search, Edit2, ChevronLeft, ChevronRight, X, Wrench } from 'lucide-react';
import toast from 'react-hot-toast';

type Role = 'USER' | 'ENGINEER' | 'ADMIN' | 'SUPER_ADMIN';
interface EngineerItem { id: string; name: string; email: string; role: Role; isActive: boolean; department?: { name: string } | null; createdAt: string; _count: { createdTickets: number }; }
...
export default function AdminEngineersPage() {
  ...
  const [editUser, setEditUser] = useState<EngineerItem | null>(null);
  ...
  const fetchEngineers = async () => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ page: String(page), limit: '25', role: 'ENGINEER' });
  ...
  const openEdit = (u: EngineerItem) => { setEditUser(u); setEditRole(u.role); setEditActive(u.isActive); };
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap gap-3 bg-card border border-border rounded-xl p-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search engineers..." className="w-full pl-8 pr-3 py-2 bg-muted border border-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="border-b border-border bg-muted/50">
              <tr>
                {['Name', 'Email', 'Role', 'Department', 'Tickets Created', 'Status', 'Actions'].map(h => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md mx-4 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-lg">Edit Engineer</h3>
